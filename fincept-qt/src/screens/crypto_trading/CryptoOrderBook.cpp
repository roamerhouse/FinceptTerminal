...
CryptoOrderBook::CryptoOrderBook(QWidget* parent) : QWidget(parent) {
    ...
    auto* title = new QLabel("订单簿");
    title->setObjectName("cryptoObTitle");
    h_layout->addWidget(title);
    h_layout->addStretch();

    const char* mode_labels[] = {"订单簿", "容量", "失衡", "信号"};
    for (int i = 0; i < 4; ++i) {
...
void CryptoOrderBook::set_data(const QVector<QPair<double, double>>& bids, const QVector<QPair<double, double>>& asks,
    ...
    spread_label_->setText(QString("价差  %1  (%2%)").arg(spread, 0, 'f', 2).arg(spread_pct, 0, 'f', 4));
    cache_dirty_ = true;
    if (repaint_timer_ && !repaint_timer_->isActive())
        repaint_timer_->start();
}
...
void CryptoOrderBook::rebuild_cache() {
    ...
    if (view_mode_ == ObViewMode::Book || view_mode_ == ObViewMode::Volume) {
        ...
        // Column headers
        p.setPen(kTextDim());
        p.drawText(QRect(4, 0, price_col_w, ROW_H), Qt::AlignLeft | Qt::AlignVCenter, "买盘");
        p.drawText(QRect(4, 0, half_w - 4, ROW_H), Qt::AlignRight | Qt::AlignVCenter, "数量");
        p.drawText(QRect(half_w + 4, 0, price_col_w, ROW_H), Qt::AlignLeft | Qt::AlignVCenter, "卖盘");
        p.drawText(QRect(half_w + 4, 0, half_w - 8, ROW_H), Qt::AlignRight | Qt::AlignVCenter, "数量");
        ...
        p.setPen(kTextDim());
        if (is_signals) {
            p.drawText(QRect(4, 0, w / 3, ROW_H), Qt::AlignLeft | Qt::AlignVCenter, "时间");
            p.drawText(QRect(w / 3, 0, w / 3, ROW_H), Qt::AlignRight | Qt::AlignVCenter, "上涨%");
            p.drawText(QRect(2 * w / 3, 0, w / 3 - 4, ROW_H), Qt::AlignRight | Qt::AlignVCenter, "操作");
        } else {
            p.drawText(QRect(4, 0, w / 3, ROW_H), Qt::AlignLeft | Qt::AlignVCenter, "时间");
            p.drawText(QRect(w / 3, 0, w / 3, ROW_H), Qt::AlignRight | Qt::AlignVCenter, "失衡度");
            p.drawText(QRect(2 * w / 3, 0, w / 3 - 4, ROW_H), Qt::AlignRight | Qt::AlignVCenter, "信号");
        }
