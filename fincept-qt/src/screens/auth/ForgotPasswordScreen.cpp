...
void ForgotPasswordScreen::build_email_page() {
    ...
    auto* sub = new QLabel("请输入您的电子邮箱，我们将向您发送验证码。");
    ...
    auto* send_btn = new QPushButton("  发送验证码  ");
    ...
    auto* back_login = new QPushButton("记起密码了？立即登录");
...
void ForgotPasswordScreen::build_otp_sent_page() {
    ...
    auto* sub = new QLabel("我们已发送验证码。请在下一步中输入该验证码以重置密码。");
    sub->setWordWrap(true);
    sub->setStyleSheet(muted_style());
    vl->addWidget(sub);
    ...
    auto* cont_btn = new QPushButton("  我已收到验证码  ");
...
void ForgotPasswordScreen::build_reset_page() {
    ...
    otp_input_ = add_field("验证码", "输入邮箱收到的验证码");
    new_password_ = add_field("新密码", "至少 8 个字符", true);
    confirm_password_ = add_field("确认新密码", "请再次输入新密码", true);
    ...
    auto* reset_btn = new QPushButton("  立即重置  ");
...
void ForgotPasswordScreen::build_success_page() {
    ...
    auto* sub = new QLabel("您的密码已成功重置。您现在可以使用新密码登录。");
    sub->setWordWrap(true);
    ...
    auto* login_btn = new QPushButton("  前往登录  ");
