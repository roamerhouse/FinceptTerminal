...
void RegisterScreen::build_form_page() {
    ...
    fn_col->setSpacing(2);
    add_field(first_name_, "名", "名", fn_col);
    nrl->addLayout(fn_col);

    auto* ln_col = new QVBoxLayout;
    ln_col->setSpacing(2);
    add_field(last_name_, "姓", "姓", ln_col);
    nrl->addLayout(ln_col);
    vl->addWidget(name_row);

    add_field(email_, "电子邮箱", "user@domain.com", vl);
    ...
    cc_col->setSpacing(2);
    add_field(country_code_, "区号", "+86", cc_col);
    country_code_->setFixedWidth(72);
    phl->addLayout(cc_col);

    auto* ph_col = new QVBoxLayout;
    ph_col->setSpacing(2);
    add_field(phone_, "电话号码", "138 0000 0000", ph_col);
    phl->addLayout(ph_col);

    vl->addWidget(ph_row);

    add_field(password_, "登录密码", "至少 8 个字符", vl);
    ...
    add_field(confirm_pw_, "确认密码", "请再次输入密码", vl);
    ...
    register_btn_ = new QPushButton("  立 即 注 册  ");
    ...
    auto* have = new QLabel("已经有账号了？");
    have->setStyleSheet(muted_style());
    lrl->addWidget(have);

    auto* signin = new QPushButton("登录");
...
void RegisterScreen::build_otp_page() {
    ...
    auto* lbl = new QLabel("验证码");
    lbl->setStyleSheet(label_style());
    vl->addWidget(lbl);

    otp_input_ = new QLineEdit;
    otp_input_->setPlaceholderText("输入邮箱收到的验证码");
    ...
    verify_btn_ = new QPushButton("  立即验证  ");
    ...
    auto* resend = new QPushButton("没收到？重新发送");
    ...
    auto* back2 = new QPushButton("返回修改资料");
...
void RegisterScreen::on_register() {
    ...
    if (fn.isEmpty() || ln.isEmpty() || em.isEmpty() || ph.isEmpty() || pw.isEmpty() || cpw.isEmpty()) {
        error_label_->setText("请填写所有必填项");
        error_label_->show();
        return;
    }
    if (cc.isEmpty()) {
        error_label_->setText("请输入区号 (例如 +86, +1)");
        error_label_->show();
        return;
    }
    ...
    if (pw != cpw) {
        error_label_->setText("两次输入的密码不一致");
        error_label_->show();
        return;
    }
    if (pw.length() < 8) {
        error_label_->setText("密码长度至少为 8 位");
        error_label_->show();
        return;
    }
    ...
    if (username.length() < 3 || username.length() > 50) {
        error_label_->setText("用户名必须在 3-50 个字符之间");
        error_label_->show();
        return;
    }

    register_btn_->setEnabled(false);
    register_btn_->setText("  正在创建...  ");
...
void RegisterScreen::on_verify_otp() {
    ...
    if (code.isEmpty()) {
        otp_error_->setText("请输入验证码");
        otp_error_->show();
        return;
    }
    verify_btn_->setEnabled(false);
    verify_btn_->setText("  正在验证...  ");
